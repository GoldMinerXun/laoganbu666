...
Page({
  ...
  searchTag: function() {
    ...
    questions.where({
      ...
      if (res.data.length == 0) {
        return wx.showToast({
          title: '没有你想搜索的内容哦，换个标签吧',
          icon: 'none'
        })
      } else {
        return that.setData({
          hot: res.data.reverse()
        })
      }
    }).then(res => {
      that.data.tag = []
  ...
  bindMultiPickerChange: function(e) {
    ...
    this.data.tag.push(academic)
    this.data.tag.push(major)
    this.data.tag.push(subject)
    this.searchTag();
    this.setData({
  ...
  onReady: function() {
    ...
    wx.cloud.callFunction({
      ...
      complete: res => {
        ...
        //   that.data.modalarr.push(res.result.data[i]._id)
        //   console.log(that.data.modalarr[i])
        //   wxParse.wxParse(res.result.data[i]._id, 'html', res.result.data[i].content, that, 0);
        // }
